...
import { validateHolidayBlocking } from './validation/holidayValidation';
import { validateEmployeeAvailability } from './validation/employeeAvailability';
...
export async function createAppointment(appointmentData: AppointmentFormData): Promise<Appointment> {
  try {
    ...
    // NOVA VALIDAÇÃO 1: Verificar feriados
    ...
    if (holidayError) {
      ...
      const mappedHolidays = holidays.map((item: any) => ({
        id: item.id,
        date: item.date,
        name: item.name,
        type: item.type,
      ...
      const holidayValidation = validateHolidayBlocking(mappedHolidays, startTime, endTime);
      if (holidayValidation.isBlocked) {
        throw new Error(holidayValidation.reason || 'Agendamento bloqueado devido a feriado');
      }
    }

    // NOVA VALIDAÇÃO 2: Verificar disponibilidade do profissional
    const availabilityValidation = await validateEmployeeAvailability(employee, date, startTime, endTime);
    if (!availabilityValidation.isAvailable) {
      throw new Error(availabilityValidation.reason || 'Profissional não está disponível neste horário');
    }
